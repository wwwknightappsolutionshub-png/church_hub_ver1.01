...
export function CongregantsOverviewStats({ stats, canViewDirectory = true }: Props) {
  const cards = [
    {
      label: 'Members',
      ...
      icon: Users,
      href: canViewDirectory ? CONGREGANTS_ROUTES.members : null,
    ...
    },
    {
      ...
      icon: Home,
      href: canViewDirectory ? CONGREGANTS_ROUTES.families : null,
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" data-testid="membership-dashboard-stats">
      {cards.map(({ label, value, hint, icon: Icon, href, accent }) => {
        ...
        const className =
          'group relative overflow-hidden rounded-xl border border-slate-200/80 bg-white p-5 shadow-sm transition hover:border-slate-300 hover:shadow-md dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-slate-700';
        const body = (
          <>
            <div
        ...
        if (href) {
          return (
            <Link key={label} href={href} data-testid={testId} className={className}>
              {body}
            </Link>
          );
        }
        return (
          <div key={label} data-testid={testId} className={className}>
            {body}
          </div>
        );
      })}
