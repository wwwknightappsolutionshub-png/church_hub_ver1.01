/** Branded celebration templates for database seed (mirrors celebration-email-shell.ts). */
const CELEBRATION_BRAND = '#1e3a5f';
const CELEBRATION_ACCENT = '#c9a227';
const CELEBRATION_BIRTHDAY_ACCENT = '#d97706';
const CELEBRATION_ANNIVERSARY_ACCENT = '#7c3aed';
...
function celebrationEmailShell(params: {
  ...
  const accent = params.accentColor ?? CELEBRATION_ACCENT;
  return `<!DOCTYPE html>
<html lang="en">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f6f9;padding:32px 16px;">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(30,58,95,0.08);">
        <tr>
          <td style="background:linear-gradient(135deg,${CELEBRATION_BRAND} 0%,#2d5a87 100%);padding:36px 32px;text-align:center;">
...
function celebrationHighlightBox(html: string, accent = CELEBRATION_ACCENT): string {
...
export const BRANDED_BIRTHDAY_BODY = celebrationEmailShell({
  eyebrow: 'Celebration Emails',
  headline: 'Happy Birthday, {{firstName}}!',
  subhead: 'We are grateful for you in our church family',
  accentColor: CELEBRATION_BIRTHDAY_ACCENT,
  bodyHtml: `${celebrationHighlightBox(
    `<p style="margin:0;font-size:18px;font-weight:600;color:${CELEBRATION_BRAND};">Dear {{fullName}},</p>`,
    CELEBRATION_BIRTHDAY_ACCENT,
  )}
...
export const BRANDED_ANNIVERSARY_BODY = celebrationEmailShell({
  eyebrow: 'Celebration Emails',
  headline: 'Celebrating {{occasionName}}',
  subhead: '{{occasionDate}} · {{fullName}}',
  accentColor: CELEBRATION_ANNIVERSARY_ACCENT,
  bodyHtml: `${celebrationHighlightBox(
    `<p style="margin:0;font-size:18px;font-weight:600;color:${CELEBRATION_BRAND};">Dear {{fullName}},</p>
     <p style="margin:8px 0 0;font-size:14px;color:#64748b;">{{occasionName}} · {{occasionDate}}</p>`,
    CELEBRATION_ANNIVERSARY_ACCENT,
  )}
